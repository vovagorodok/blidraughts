...
import { State, makeDefaults } from '../../draughtsground/state'
...
export class CppProtocol extends BaseProtocol {
  ...
  _variantsMap = {
    ...
    antidraughts: this._variants.antidraughts,
    breakthrough: this._variants.breakthrough,
    frisian: this._variants.frisian,
    frysk: this._variants.frysk,
    russian: this._variants.russian,
    brazilian: this._variants.brazilian,
    english: this._variants.english,
  }
  _endReasonsMap = {
...
class RoundOngoing extends Round {
  ...
  onPeripheralCommand(cmd: string) {
    if (cmd.startsWith(Command.Move)) {
      ...
      if (!sendMoveToCentral(state, move))
        this.context.onMoveRejectedByCentral();
    }
    else super.onPeripheralCommand(cmd)
  }
}
