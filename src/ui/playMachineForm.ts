    ...
    generalFieldset.push(h('div.setupPosition', fromPositionFen ? [
      h('div.setupMiniBoardWrapper', {
        ...
        oncreate: helper.ontap(() => {
          close()
          if (fromPositionFen) {
            router.goTo(`/editor/${encodeURIComponent(fromPositionFen)}`)
          }
        })
      }, [
        h(ViewOnlyBoard, { fen: fromPositionFen, variant: getVariantKeyById(settingsObj.variant()) || 'standard', orientation: 'white'})
      ])
