...
const lidraughtsSourcePath = `${baseDir}/source`
const lidraughtsTranslationsPath = `${baseDir}/lidraughts/translation/dest`
...
const modules = ['arena', 'challenge', 'perfStat', 'preferences', 'search', 'settings', 'site', 'study', 'team', 'tfa']
...
async function main() {
  ...
  // Download source translations (en-GB) from lidraughts repo
  await downloadLidraughtsSourcesTo(baseDir)

  // en-GB is the source locale, thus it has a special treatment
  const locales = ['en-GB'].concat(readdirSync(`${lidraughtsTranslationsPath}/site`)
  .map(fn => fn.split('.')[0]))
...
async function downloadTranslationsTo(zipFile) {
  ...
  const response = await fetch('https://crowdin.com/backend/download/project/lidraughts.zip')
...
async function downloadLidraughtsSourcesTo(dir) {
  console.log(colors.blue('Downloading lidraughts source translations...'))
...
function loadTranslations(dir, locale) {
  if (locale === 'en-GB')
    return parseStringPromise(
      readFileSync(`${lidraughtsSourcePath}/${dir}.xml`)
    )
  else
    return parseStringPromise(
      readFileSync(`${lidraughtsTranslationsPath}/${dir}/${locale}.xml`)
    )
}
